...
mod tests {
    ...
    #[test]
    fn calls_concrete_block() {
        let block = ConcreteBlock::new(|value: i32| value + 5);
        assert_eq!(unsafe { block.call((6,)) }, 11);
    }

    #[test]
    fn copies_concrete_block() {
        let text = "Hello!".to_string();
        let expected = text.len() as i32;
        let block = ConcreteBlock::new(move || text.len() as i32).copy();
        assert_eq!(unsafe { block.call(()) }, expected);
    }
}
